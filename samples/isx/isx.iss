...
function  __isx_setuponly_VerFormat(version: PAnsiChar; pad: Integer): PAnsiChar;
external  'VerFormat@files:isx.dll stdcall setuponly';
function  __isx_uninstallonly_VerFormat(version: PAnsiChar; pad: Integer): PAnsiChar;
external  'VerFormat@{app}\._unins000.isx\isx.dll stdcall uninstallonly';
...
begin
  if (not isInitDone) then RaiseException('ISX not initialized');
  if (isSetup) then begin 
    result := __isx_setuponly_VerFormat(version, nbDigits);
  end else begin 
    result := __isx_uninstallonly_VerFormat(version, nbDigits);
  end;
end;
